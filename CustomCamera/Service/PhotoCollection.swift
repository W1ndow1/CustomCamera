...
class PhotoCollection: NSObject, ObservableObject {
    ...
    func load() async throws {
        ...
        if let smartAlbumType = smartAlbumType {
            if let assetCollection = PhotoCollection.getSmartAlbum(subtype: smartAlbumType) {
                ...
                logger.log("Unable to load smart albumof type: \(smartAlbumType.rawValue)")
                throw PhotoCollectionError.unableToLoadSmartAlbum(smartAlbumType)
            }
        }
        guard let name = albumName, !name.isEmpty else {
            logger.log("Unalbe ot load an album without a name")
    ...
    func addImage(_ imageDate: Data) async throws {
        ...
        do {
            try await PHPhotoLibrary.shared().performChanges {
                ...
                if let assetPlaceholder = creationRequest.placeholderForCreatedAsset {
                    creationRequest.addResource(with: .photo, data: imageDate, options: nil)
    ...
    private static func getSmartAlbum(subtype: PHAssetCollectionSubtype) -> PHAssetCollection? {
        let fetchOptions = PHFetchOptions()
        let collection = PHAssetCollection.fetchAssetCollections(with: .smartAlbum, subtype: subtype, options: fetchOptions)
    ...
    private static func createAlbum(named name: String) async throws -> PHAssetCollection? {
        ...
        do {
            ...
            logger.error("Error creating album is photo library: \(error.localizedDescription)")
        ...
        let collection = PHAssetCollection.fetchAssetCollections(withLocalIdentifiers: [collectionIdentifier], options: nil)
        return collection.firstObject
    }
